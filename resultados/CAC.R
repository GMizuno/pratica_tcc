...
p3 <- ggplot(CAC, aes(x = Index, y = cac)) +
  ...
  theme_minimal() + 
  geom_vline(xintercept = as.Date(c("2007-07-01", "2008-09-22", "2008-11-28")), 
...
dummy2 <- as.matrix(dummy_on_off(n, c(1, 899, 1226),
                                 c(898, 1212, n)))
...
(opt3 <- estimando(llike_arch, pars))

media_cond <- esp_cond_arch(
  data = yt,
  est = opt3,
  dummy1 = dummy1,
  alpha_order = alpha_order,
...
dummy1 <- as.matrix(dummy_step(n, 1, "Media"))
dummy2 <- as.matrix(dummy_on_off(n, c(1, 899),
                                 c(898, n)))
# Ordens e Parametros - FIM
